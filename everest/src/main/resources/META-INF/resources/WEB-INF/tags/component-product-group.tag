		...
		<div>
			<div style="float: left; width: calc(100% - 121px); margin-right: 5px;">
				<input type="text" name=${name}" class="form-control input-sm"
					placeholder="Select a Product Group" disabled="disabled" autocomplete="off" 
					value="${value} " />
			</div>

			<div style="float: left">
				<div ng-controller="ComponentProductGroupController as componentProductGroup">
					<button type="button" ng-click="componentProductGroup.showProductGroupDialog($event, '${dialog}')" class="btn btn-success btn-sm">Search</button>
