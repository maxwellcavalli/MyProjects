...
angular.module('myApp').controller('ProductSubGroupController', ['$scope', '$log', '$rootScope', 'CrudService', 'ConfirmationDialog', 'HOST', 
	...
	self.datatable.header.push(new DatatableHeader('Group', '30%', 'left'));
    ...
    self.cleanProductGroup 	= cleanProductGroup;
    
    function selectProductGroup(d){
    	$log.info(d);
    	self.domain.product_sub_group_group = d;
    }
    
    function cleanProductGroup(){
    	self.domain.product_sub_group_group = null;
    }
    ...
    var REST_SERVICE_URI 	= HOST + '/productSubGroup/';
    var REST_REPOSITORY_URI = HOST + '/api/productSubGroupRepo/search/findByNameLikeIgnoreCase?name='
    ...
    function submit() {
        if(self.domain.product_sub_group_id===null){
            $log.info('Saving New Domain', self.domain);
            create(self.domain);
        }else{
            update(self.domain, self.domain.product_sub_group_id);
            $log.info('Domain updated with code ', self.domain.product_sub_group_id);
        }
    }
    ...
    function edit(code){
        ...
        for(var i = 0; i < self.datatable.data.length; i++){
            if(self.datatable.data[i].product_sub_group_id === code) {
    ...
    function remove(code){
        ...
        if(self.domain.product_sub_group_id === code) {
            reset();
        }
