...
angular.module('myApp').controller('ProductGroupController', ['$scope', '$log', '$rootScope', 'CrudService', 'ConfirmationDialog', 'HOST', 
	...
	self.datatable.header.push(new DatatableHeader('Name', '80%', 'left', ''));
    ...
    var REST_SERVICE_URI 	= HOST + '/productGroup/';
    var REST_REPOSITORY_URI = HOST + '/api/productGroupRepo/search/findByNameLikeIgnoreCase?name='
    ...
    function submit() {
        if(self.domain.product_group_id===null){
            $log.info('Saving New Domain', self.domain);
            create(self.domain);
        }else{
            update(self.domain, self.domain.product_group_id);
            $log.info('Domain updated with code ', self.domain.product_group_id);
        }
    }
    ...
    function edit(code){
        ...
        for(var i = 0; i < self.datatable.data.length; i++){
            if(self.datatable.data[i].product_group_id === code) {
    ...
    function remove(code){
        ...
        if(self.domain.product_group_id === code) {
            reset();
        }
