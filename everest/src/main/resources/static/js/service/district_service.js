...
angular.module('myApp').factory('DistrictService', ['$http', '$q', '$log', 'HOST', function($http, $q, $log, HOST){
 
    var REST_SERVICE_URI 	= HOST + '/district/';
    var REST_REPOSITORY_URI = HOST + '/api/districts/search/findByNameLikeIgnoreCase?name='
    ...
    function fetchAll(pageNumber, pageSize, filter) {
        ...
        fetchAllService(pageNumber, pageSize, filter).then(
            function (response) {
            	var records = response.data._embedded.districts;
